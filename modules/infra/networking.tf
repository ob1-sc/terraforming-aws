...
data "template_file" "infrastructure_subnet_reserved" {
  # Render the template once for each availability zone
  count    = "${length(var.availability_zones)}"
  template = "$${reserved}"

  vars {
    reserved = "${cidrhost(element(aws_subnet.infrastructure_subnets.*.cidr_block, count.index), 1)}-${cidrhost(element(aws_subnet.infrastructure_subnets.*.cidr_block, count.index), 9)}"
  }
}
